            ...
            if(command.contains("merge")) {
                ...
                if(startIndex >= 0 && startIndex <= texts.size() - 1 && endIndex >= 0 && endIndex <= texts.size()-1) {
                    ...
                    if(index >= 0 && index <= texts.size() - 1) {
                        String textForDriver = texts.get(index);
                        texts.remove(index);

                        int countSymbolsPerPart = textForDriver.length() / parts;

                        int beginIndex = 0;
                        for (int part = 1; part < parts; part++) {
                            String textPerPart = textForDriver.substring(beginIndex, beginIndex + countSymbolsPerPart);
                            texts.add(index, textPerPart);
                            index++;
                            beginIndex += countSymbolsPerPart;
                        }
                        String textLastPart = textForDriver.substring(beginIndex, textForDriver.length());
                        texts.add(index, textLastPart);
                }
            }
